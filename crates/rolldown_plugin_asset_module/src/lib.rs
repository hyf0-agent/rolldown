...
impl AssetModulePlugin {
  async fn load_impl(
    ...
    let is_asset = args.asserted_module_type.is_some_and(|ty| matches!(ty, ModuleType::Asset))
      || self.is_asset_by_extension(args.id);
    ...
    let bytes = tokio::fs::read(id)
      .await
      .map_err(|e| anyhow::anyhow!("Failed to read asset module {id}: {e}"))?;
    ...
    // can look up the asset filename by module ID
    let module_id: ArcStr = id.into();
    ctx.associate_module_with_emitted_file(module_id, reference_id.clone());

    // Add watch file for watch mode
    ctx.add_watch_file(id);

    // Return JS code that exports the asset placeholder.
    // The placeholder will be resolved in renderChunk to the actual filename.
    let code = format!("export default \"{PREFIX}{reference_id}\"");
