...
pub async fn load_source<Fs: FileSystem + 'static>(
  ...
  // If we're given a specific module type to use and no plugin has already handled loading,
  // apply the asserted type. When a plugin's load hook returns a module_type (e.g. the asset
  // module plugin returns Js after handling the asset), we trust the plugin's decision.
