...
pub struct FileEmitter {
  ...
  module_to_emitted_file: FxDashMap<ArcStr, ArcStr>,
}
...
impl FileEmitter {
  pub fn new(options: Arc<NormalizedBundlerOptions>) -> Self {
    Self {
      ...
      module_to_emitted_file: DashMap::default(),
    }
  }
  ...
  pub fn associate_module_with_emitted_file(&self, module_id: ArcStr, reference_id: ArcStr) {
    self.module_to_emitted_file.insert(module_id, reference_id);
  }

  /// Get the emitted file reference ID for a given module ID.
  pub fn get_emitted_file_for_module(&self, module_id: &str) -> Option<ArcStr> {
    self.module_to_emitted_file.get(module_id).map(|v| v.value().clone())
  }
  ...
  pub fn clear(&self) {
    ...
    self.module_to_emitted_file.clear();
  }
}
